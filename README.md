...
**netbox MCP**: gives Claude Code access to the homelab's NetBox instance, for querying and automating IPAM/DCIM data (devices, IPs, racks, cabling, etc.) directly from a Claude Code session instead of the NetBox UI/API by hand. Only reachable from the home LAN. Connection details and the bearer token are kept out of this file — see `run_onchange_30-configure-mcp.sh` and the encrypted secret next to it.
